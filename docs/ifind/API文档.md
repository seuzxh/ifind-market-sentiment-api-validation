...
异常跳变必须先分段再分析。本轮将2026-05-25冻结为全局边界：第一段数据截至2026-05-22，第二段数据从2026-05-26开始；边界日及所有跨边界收益、滚动窗口、趋势、Spread、相关性和回溯均剔除。窗口不足时置空并标记`SEGMENT_WARMUP`，不能跨段补齐。供应商修订后以新revision重算，保留旧分段结果。
...
2026-09-09复验：`reportname=p00112`，`functionpara={"sdate":"20250101","edate":"20260908","p0":"A股"}`，`outputpara`为`p00112_f001`至`p00112_f014`的逗号分隔列表。返回HTTP200、`errorcode=0`，共409个交易日，2025-01-02至2026-09-08，与交易日历和主基准逐日匹配。**请求成功不等于覆盖沪深京；不使用AB替代。**

| 字段 | 返回字段名/解释 | 本轮状态 |
|---|---|---|
| `p00112_f001` | 日期，YYYY/MM/DD | 409日完整返回 |
| `p00112_f002/f003/f004` | 沪深两市上涨/平盘/下跌 | 409日均等于沪市加深市，未体现北交所 |
| `p00112_f005/f006/f007` | 沪市上涨/平盘/下跌 | 已返回，参与加总核对 |
| `p00112_f008/f009` | 上综指涨跌幅/收盘 | 已返回，不用于本轮广度实验 |
| `p00112_f010/f011/f012` | 深市上涨/平盘/下跌 | 已返回，参与加总核对 |
| `p00112_f013/f014` | 深成指涨跌幅/收盘 | 已返回，不用于本轮广度实验 |
| `p00112_f021/f022/f023` | 创业板上涨/平盘/下跌 | 深市子集，不能重复加总 |
| `p00112_f024/f025` | 创业板指涨跌幅/收盘 | 未复验 |

响应`outParams`明确标注“沪深两市”，且每一天都满足`f002=f005+f010`、`f003=f006+f011`、`f004=f007+f012`。2026-09-10用户确认涨跌家数只需沪深纯A，北交所不再是前置要求。保留`A_candidate_SH_SZ`用于来源追溯，当前研究按沪深范围开展。原始脱敏响应保存于本地`data/breadth_evidence/`，汇总和复现记录见[广度确认实验](../agent/广度确认实验.md)。

研究中使用`UpRatio=U/(U+F+D)`、`NetBreadth=(U-D)/(U+F+D)`，平盘计入分母；广度确认要求`UpRatio>0.5`，恰好50%不通过。缺失、负数、非整数、非有限数、零总数或重复日期不参与成对实验比较，并单独计数。停牌按用户要求不作为过滤条件；历史发布时间与修订机制保留为数据维护事项，不要求北交所历史字段。不能从这些字段推导涨停/跌停家数，不前填缺失日期，也不把旧日广度拼成新日特征。
