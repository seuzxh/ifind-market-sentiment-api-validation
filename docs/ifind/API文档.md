...
响应`outParams`明确标注“沪深两市”，且每一天都满足`f002=f005+f010`、`f003=f006+f011`、`f004=f007+f012`。因此仅可按`A_candidate_SH_SZ`候选沪深子集研究，不能标为沪深京全市场。原始脱敏响应保存于本地`data/breadth_evidence/`，汇总和复现记录见[广度确认实验](../agent/广度确认实验.md)。

研究中使用`UpRatio=U/(U+F+D)`、`NetBreadth=(U-D)/(U+F+D)`，平盘计入分母；广度确认要求`UpRatio>0.5`，恰好50%不通过。缺失、负数、非整数、非有限数、零总数或重复日期不参与成对实验比较，并单独计数。停牌按用户要求不作为过滤条件；北交所历史字段、纯A枚举语义和历史发布时间仍需供应商说明。不能从这些字段推导涨停/跌停家数，不前填缺失日期，也不把旧日广度拼成新日特征。
